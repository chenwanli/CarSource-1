...
-(id)init
{
    ...
    if(self){
        ...
        _selectedColor = [UIColor colorWithRed:18/255.0 green:152/255.0 blue:234/255.0 alpha:1];
        _linePercent = 0.8;
        _lineHieght = 2.5;
    }
    return self;
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        if(i==_btnTag){
           ...
           [btn setTitleColor:_config.selectedColor forState:UIControlStateNormal];
                   self.line = [[UIImageView alloc] initWithFrame:CGRectMake(_config.itemWidth*(1-_config.linePercent)/2.0 +5 + SCREEN_WIDTH/4 *i, CGRectGetHeight(self.frame) - 20, _config.itemWidth*_config.linePercent-15, 19)];
            _line.image = [UIImage imageNamed:@"she"];
            [self addSubview:_line];
        }
    }
    UIView *xView = [[UIView alloc]initWithFrame:CGRectMake(0, 49, self.frame.size.width, 1)];
    xView.backgroundColor = [UIColor colorWithRed:18/255.0 green:152/255.0 blue:234/255.0 alpha:1];
...
-(void)changeLine:(float)index
{
    ...
    rect.origin.x = index*_config.itemWidth  + _config.itemWidth*(1-_config.linePercent)/2.0+10;
    _line.frame = rect;
}
