...
- (void)updateSession:(ECSession*)session{
    ...
    [self.dataBase executeUpdate:@"INSERT INTO session(sessionId, dateTime, type, text, unreadCount) VALUES (?,?,?,?,?)",session.sessionId, @(session.dateTime), @(session.type), session.text, @(session.unreadCount)];
}
...
- (NSMutableDictionary *)loadAllSessions {
    ...
    FMResultSet *rs = [self.dataBase executeQuery:@"SELECT sessionId, dateTime, type, text, unreadCount, sumCount FROM session ORDER BY dateTime DESC"];
    while ([rs next]) {
...
-(BOOL)isDiscussOfGroupId:(NSString *)groupId {
    ...
    if ([rs next]) {
        isGroup = [rs boolForColumnIndex:0];
    }
    [rs close];
