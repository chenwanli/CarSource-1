...
@interface RulesCarView ()<UITextViewDelegate,UITableViewDataSource,UITableViewDelegate>{
...
- (void)initRulesCarRulesTableFooterView{
    ...
    NSArray *footerArray = [[NSArray alloc]initWithObjects:@"  谁可以看",@"  会员类型", nil];
    for (int i = 0; i<footerArray.count; i++) {

        if (i == 0) {
            UILabel *footerLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 44*i +10, SCREEN_WIDTH, 44)];
            footerLabel.font = [UIFont systemFontOfSize:16];
            footerLabel.layer.borderColor = [UIColor colorWithRed:236/255.0 green:246/255.0 blue:254/255.0 alpha:1].CGColor;
            footerLabel.layer.borderWidth = 1.0;
            footerLabel.text = [footerArray objectAtIndex:i];
            ...
            UIButton *membersButton  = [UIButton buttonWithType:UIButtonTypeCustom];
            [membersButton setTitle:[footerArray objectAtIndex:i] forState:UIControlStateNormal];
            membersButton.frame = CGRectMake(0,44*i +10, SCREEN_WIDTH, 44);
            membersButton.titleLabel.font = [UIFont systemFontOfSize:16];
            membersButton.layer.borderColor = [UIColor colorWithRed:175/255.0 green:219/255.0 blue:246/255.0 alpha:1].CGColor;
            membersButton.layer.borderWidth = 1.0;
            membersButton.backgroundColor = [UIColor whiteColor];
            membersButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
            [membersButton setTitleColor:[UIColor grayColor] forState:UIControlStateNormal];
            [membersButton addTarget:self action:@selector(membersButtonClick:) forControlEvents:UIControlEventTouchUpInside];
            [footerView addSubview:membersButton];
            ...
            [footerView addSubview:milledImage];
        ...
        depositButton.frame = CGRectMake(50,150 + 60 *i, SCREEN_WIDTH - 100, 44);
